...
- "tool": MUST be exactly one of: "shell" or "filesystem" (no other values allowed)
...
def build_replan_prompt(request: CognitionRequest) -> str:
    return (
        ...
        "Create a revised plan that avoids the previous failure. Use only 'shell' or 'filesystem' tools."
    )
